                        ...
                        <div class="card-header">
                            <div class="row">
                                <div class="col-10">
                                    ...
                                    <h4 class="page-title">{{ ucwords($menuNames['priority_' . $company_id] ??  $title )}}</h4>
                                </div>
                             
                                <div class="col-2 text-end">
                                    <a wire:navigate class="btn btn-primary" href="{{ route('priority.create') }}"><i class="fa fa-plus"></i> {{ __('Add New') }}</a>
                                </div>
                              
                            </div>
                        </div>
                        
                            <form class="mb-15" id="kt_course_delete" action="{{ route('priority.delete') }}" method="POST">
                                @csrf
                                @method('post')
                                ...
                                data-column="id,name,sequence_number,created_at,created_by{{ auth()->user()->role == 'super-admin' ? ',company_name' : '' }},status"
                                data-column-name="id,name,sequence_number,created_at,created_by{{ auth()->user()->role == 'super-admin' ? ',company_name' : '' }},status,actions"
                                data-extra-param="filter_active,filter_company"
                                data-extra-param-name="status,company_name"
                                data-control="{{ route('priority.list') }}" data-sorting="3" data-sorting-type="DESC"
                                ...
                                role="grid" aria-describedby="common_datatable_info">
                                    ...
                                    <thead>
                                        ...
                                        <tr>
                                            ...
                                            <th>{{ ucwords($menuNames['priority_' . $company_id] ??  'Priority') }} Name</th>
