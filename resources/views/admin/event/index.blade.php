                        ...
                        <div class="card-header">
                            <div class="row">
                                <div class="col-10">
                                    ...
                                    <h4 class="page-title">{{ ucwords($menuNames['event_' . $company_id] ?? $title) }}</h4>
                                </div>
                                @can('event.create')
                                <div class="col-2 text-end">
                                    <a wire:navigate class="btn btn-primary" href="{{ route('event.create') }}"><i class="fa fa-plus"></i> {{ __('Add New') }}</a>
                                </div>
                                @endcan
                            ...
                            <form class="mb-15" id="kt_course_delete" action="{{ route('event.delete') }}" method="POST">
                                @csrf
                                @method('post')
                                ...
                                <table id="common_datatable" class="table table-striped w-100"
                                    data-column="id,name,description,sequence_number{{ auth()->user()->role == 'super-admin' ? ',company_name' : '' }},created_at,status"
                                    data-column-name="id,name,description,sequence_number{{ auth()->user()->role == 'super-admin' ? ',company_name' : '' }},created_at,status,actions"

                                    data-extra-param="filter_active,filter_company"
                                    data-extra-param-name="status,company_id"

                                    data-control="{{route('event.list')}}" data-sorting="4" data-sorting-type="DESC"
